                    ...
                    {dsHocPhan.map((hocphan, index) => (
                        <tr key={index} className="odd:bg-white odd:dark:bg-gray-900 even:bg-gray-50 even:dark:bg-gray-800 border-b dark:border-gray-700">
                            <td scope="row" className="px-6 py-4 font-medium text-gray-900 whitespace-nowrap dark:text-white ">
                                {hocphan?._id?.substring(hocphan._id.length - 6)}
                            </td>
                            <td scope="row" className="px-6 py-4 font-medium text-gray-900 whitespace-nowrap dark:text-white ">
                            ...
                            <td scope="row" className="px-6 py-4 font-medium text-gray-900 whitespace-nowrap dark:text-white ">
                                {hocphan?.tietLyThuyet.map((item) => `${item.ngay ? `${item.ngay === 1 ? 'Chủ Nhật' : 'Thứ ' + item.ngay} - Tiết (${item.tiet}) - Phòng (${item.phong.tenPhong})` : 'Chưa lên lịch'}`)}
                            </td>
                            <td scope="row" className=" px-6 py-4 font-medium flex flex-col text-gray-900 whitespace-nowrap dark:text-white">
                                {hocphan?.tietThucHanh.map((item, index) => (
                                    <span>{`${hocphan?.tietThucHanh.length > 1 ? `Nhóm (${index + 1}) - ` : ''}${item.ngay ? `${item.ngay === 1 ? 'Chủ Nhật' : 'Thứ ' + item.ngay} - Tiết (${item.tiet}) - Phòng (${item.phong.tenPhong})` : 'Chưa lên lịch'}`}</span>
                                ))}
                            </td>
                            <td scope="row" className=" px-6 py-4 font-medium text-gray-900 whitespace-nowrap dark:text-white">
