                ...
                setDsThoiGianPhongDaDangKyFilter(
                    dsThoiGianPhongDaDangKy.filter(item1 => {
                        const isInFilter = dsThoiGianPhongDaDangKy.some(
                            item =>
                                item.phong.maPhong === currentHocPhan.tietLyThuyet[0].phong.maPhong &&
                                item.ngay === currentHocPhan.tietLyThuyet[0].ngay &&
                                item.tiet === currentHocPhan.tietLyThuyet[0].tiet &&
                                item === item1
                        );
                        return !isInFilter
                    })
                ...
                {currentHocPhan && (
                    <>
                        ...
                        {[...currentHocPhan.tietLyThuyet, ...currentHocPhan.tietThucHanh].map((item, index) => (
                            <button onClick={() => setType(index)} style={{ backgroundColor: type === index ? '#5bd7e5' : checkHaveRoom(item) ? '#82e0aa' : '#f2f2f2', color: type === index ? 'white' : !checkHaveRoom(item) ? 'black' : 'white' }} className='text-[13px] transition-all hover:scale-[1.05] text-[white] px-2 py-1 rounded-md'>
                        ...
                        <>
                            <div style={{ width: `${currentPhong ? '70' : '90'}%`, transition: '0.5s' }} className='flex no overflow-y-auto max-h-[98%] flex-wrap items-start gap-2 justify-center'>
                                {phongs.map((phong, index) => {
                                    if (type === 0) {
                                        if (phong.loai === 'LT') {
                                            return <div key={index} onClick={() => setCurrentPhong(phong)} style={{ backgroundColor: phong._id === currentPhong?._id ? '#82e0aa' : '#f2f2f2', fontWeight: phong._id === currentPhong?._id ? 500 : 400, color: phong._id === currentPhong?._id ? 'white' : 'black' }} className='w-[90px] aspect-square transition-all hover:scale-[1.05] flex items-center shadow-md cursor-pointer justify-center bg-[#f2f2f2] rounded-md'>
                                                {phong.tenPhong}
                                            </div>
                                        }
                                    } else {
                            ...
                            <div style={{ width: `${currentPhong ? '23' : '0'}%`, transition: '0.5s', borderLeft: currentPhong ? '1px solid #e5e5e5' : 'none', paddingLeft: currentPhong ? '8px' : 0 }} className='h-[100%] relative flex flex-col overflow-hidden'>
                                ...
                                <div className='no w-[90%] max-h-[83%] overflow-y-auto mt-1'>
                                    <div className='flex flex-col gap-[2px]'>
                                        ...
                                        <div style={{ height: currentDate === 2 ? '450px' : 0, transition: '0.5s' }} className='w-full flex flex-col overflow-hidden gap-1'>
                                        ...
                                        <div style={{ height: currentDate === 3 ? '450px' : 0, transition: '0.5s' }} className='w-full flex flex-col overflow-hidden gap-1'>
                                            {scheduleOptions.map((item, index) => {
                                                if (checkThoiGianPhong(dsThoiGianPhongDaDangKyFilter, currentPhong, currentDate, item.value)) {
                                                    return <div onClick={() => setCurrentTime(currentTime === currentDate + '/' + item.value ? '' : currentDate + '/' + item.value)} style={{ backgroundColor: currentTime === currentDate + '/' + item.value ? '#f5f5f5' : 'white' }} className='text-[12px] px-4 cursor-pointer w-[100%] py-[4px] hover:bg-[#f5f5f5] transition-all rounded-sm' key={index}>
                                                        {item.label}
                                        ...
                                        <div style={{ height: currentDate === 4 ? '450px' : 0, transition: '0.5s' }} className='w-full flex flex-col overflow-hidden gap-1'>
                                            {scheduleOptions.map((item, index) => {
                                                if (checkThoiGianPhong(dsThoiGianPhongDaDangKyFilter, currentPhong, currentDate, item.value)) {
                                                    return <div onClick={() => setCurrentTime(currentTime === currentDate + '/' + item.value ? '' : currentDate + '/' + item.value)} style={{ backgroundColor: currentTime === currentDate + '/' + item.value ? '#f5f5f5' : 'white' }} className='text-[12px] px-4 cursor-pointer w-[100%] py-[4px] hover:bg-[#f5f5f5] transition-all rounded-sm' key={index}>
                                                        {item.label}
                                        ...
                                        <div style={{ height: currentDate === 5 ? '450px' : 0, transition: '0.5s' }} className='w-full flex flex-col overflow-hidden gap-1'>
                                            {scheduleOptions.map((item, index) => {
                                                if (checkThoiGianPhong(dsThoiGianPhongDaDangKyFilter, currentPhong, currentDate, item.value)) {
                                                    return <div onClick={() => setCurrentTime(currentTime === currentDate + '/' + item.value ? '' : currentDate + '/' + item.value)} style={{ backgroundColor: currentTime === currentDate + '/' + item.value ? '#f5f5f5' : 'white' }} className='text-[12px] px-4 cursor-pointer w-[100%] py-[4px] hover:bg-[#f5f5f5] transition-all rounded-sm' key={index}>
                                                        {item.label}
                                        ...
                                        <div style={{ height: currentDate === 6 ? '450px' : 0, transition: '0.5s' }} className='w-full flex flex-col overflow-hidden gap-1'>
                                            {scheduleOptions.map((item, index) => {
                                                if (checkThoiGianPhong(dsThoiGianPhongDaDangKyFilter, currentPhong, currentDate, item.value)) {
                                                    return <div onClick={() => setCurrentTime(currentTime === currentDate + '/' + item.value ? '' : currentDate + '/' + item.value)} style={{ backgroundColor: currentTime === currentDate + '/' + item.value ? '#f5f5f5' : 'white' }} className='text-[12px] px-4 cursor-pointer w-[100%] py-[4px] hover:bg-[#f5f5f5] transition-all rounded-sm' key={index}>
                                                        {item.label}
                                        ...
                                        <div style={{ height: currentDate === 7 ? '450px' : 0, transition: '0.5s' }} className='w-full flex flex-col overflow-hidden gap-1'>
                                            {scheduleOptions.map((item, index) => {
                                                if (checkThoiGianPhong(dsThoiGianPhongDaDangKyFilter, currentPhong, currentDate, item.value)) {
                                                    return <div onClick={() => setCurrentTime(currentTime === currentDate + '/' + item.value ? '' : currentDate + '/' + item.value)} style={{ backgroundColor: currentTime === currentDate + '/' + item.value ? '#f5f5f5' : 'white' }} className='text-[12px] px-4 cursor-pointer w-[100%] py-[4px] hover:bg-[#f5f5f5] transition-all rounded-sm' key={index}>
                                                        {item.label}
                                        ...
                                        <div style={{ height: currentDate === 1 ? '450px' : 0, transition: '0.5s' }} className='w-full flex flex-col overflow-hidden gap-1'>
                                            {scheduleOptions.map((item, index) => {
                                                if (checkThoiGianPhong(dsThoiGianPhongDaDangKyFilter, currentPhong, currentDate, item.value)) {
                                                    return <div onClick={() => setCurrentTime(currentTime === currentDate + '/' + item.value ? '' : currentDate + '/' + item.value)} style={{ backgroundColor: currentTime === currentDate + '/' + item.value ? '#f5f5f5' : 'white' }} className='text-[12px] px-4 cursor-pointer w-[100%] py-[4px] hover:bg-[#f5f5f5] transition-all rounded-sm' key={index}>
                                                        {item.label}
